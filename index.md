...
אתם רשאים לתייג כמה פסקאות שתרצו, התשלום מתבצע בהתאם לכל פסקה מתויגת שאתם שולחים. בתחתית הדף יש שאלות ותשובות שיעזרו לכם במשימת התיוג.
...
### דוגמה לפסקה מתויגת עם הישויות המשפטיות: 

שימו לב זוהי דוגמה שמכילה לא מעט מהישויות שצוינו בטבלה לעיל(הדוגמה דומה לפסקאות אמיתיות שתתייגו בפסקי הדין שתעבדו עליהם, הדוגמה הותאמה להכיל עוד ישויות):

 <img src="./src/full_example_src.png"/>


### שאלות נפוצות:
* האם יש שאלה מנחה לזיהוי ישות? 
**כאשר אתם ניגשים לתייג ישות מסוימת בתג X אתם צריכים לשאול את עצמכם את השאלה 'מי ה-X?' ואם התשובה היא אותו קטע טקסט שסיווגתם כישות X אז לסמן.**
* ישנה ישות שחוזרת על עצמה, האם עליי להמשיך לתייג אותה תחת אותו תג כפי שזיהיתי? 
**כן, הסיבה לכך היא שהדבר משמעותי לפיתוח אותם מודלים שהוזכרו בפתח דבר.**
* כמה ישויות עליי לתייג בפסקה? 
**יש לתייג את כלל הישויות שמופיעות בפסקה שאותה אתם מתייגים ורק לאחר מכן(כמובן תוך כדי ווידוי שהכנסתם את המספר מזהה שלכם בהערה) להגיש ולעבור לפסקה הבאה.**
* האם קיימת מגבלת זמן או האם פלטפורמת התיוג עשויה להיסגר באמצע? 
**פלטפורמת התיוג ופרוליפיק עובדים בצורה כזו שאם לא בוצעה עבודה לאורך זמן(מחשב ומסך דומם) או שעברה כמות זמן מסוימת מרגע תחילת העבודה אז הפלטפורמה/הפרוליפיק עשויה לזהות את הסטטוס כסיום ולסיים את משימת התיוג שנשארה פתוחה. לכן יש לזכור כי אתם עובדים על בסיס כמות פסקאות מתויגות - תייגו את כמות הפסקאות בהתאם ליכולת שלכם ואז במידה ותרצו להמשיך אפשר לפנות אלינו לביצוע עבודה נוספת שוב.**
* האם קיימת היררכיה בנוגע לתיוג? כלומר אם מדינת ישראל היא גם המשיב וגם רשות שלטונית אז מה היא קודם? 
**יש לתייג את הייחודי / נדיר יותר. למשל, מדינת ישראל היא תמיד רשות שלטונית ולא תמיד היא המשיב - אז יש לתייג את הייחודי יותר.**
* האם המילה "המשיב" צריכה להיות מתוייגת כ"משיב" או רק שמות ספציפיים? כנ"ל על "השופט", "הבית המשפט" וכד'. 
**גם המילה ״המשיב״ צריכה להיות מתוייגת - בהנחה שהיא מתייחסת למשיב. יכולים להיות מקרים שכתוב ״המשיב״ והכוונה לא למשיב המשפטי. כמו למשל ׳משיב הרוח ומוריד הגשם׳**
* האם "הבורר" נחשב כ"שופט"? האם "הנאשם" נחשב כ"משיב"? 
**אם ׳הבורר׳ בטקסט המתויג הוא השופט - כלומר הגורם המכריע - אז כן. אותו דבר לגבי הנאשם (אם הוא ׳משיב׳ למדינה על כתב האישום)**
* האם שירות המבחן זה ארגון לתועלת הציבור או רשות שלטונית? 
**רשות שלטונית**
